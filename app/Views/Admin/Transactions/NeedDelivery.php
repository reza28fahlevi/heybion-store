...
<script>
  $(document).ready(function() {
    ...
    $('#f_invoice').on('submit', function(e) {
      ...
      GetData($('#transaction_id').val())
    });

    function GetData(id) {
      $('#delivery_service').val('Loading...')
      $.ajax({
        url: '<?= site_url('hb-admin/transactions/getdetail/') ?>' + id,
        type: 'GET',
        success: function(response) {
          $('#delivery_service').val('')
          $('#delivery_service').val(response.data.invoice.keterangan_ekspedisi)
        },
        error: function(xhr, status, error) {
          // Handle errors here
          Swal.fire({
    ...
    $('#f_delivery').on('submit', function(e) {
      ...
      formData.append('delivery_service', $('#delivery_service').val());
